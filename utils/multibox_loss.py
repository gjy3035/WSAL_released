...
class MultiBoxLoss(nn.Module):
    ...
    def hard_negative_mining(self, conf_loss, pos):
        ...
        num_neg = torch.clamp(3*num_pos, max=num_boxes-1).view(-1,1)  # [N,1]
        # pdb.set_trace()
    ...
    def forward(self, loc_preds, loc_targets, conf_preds, conf_targets):
        ...
        # 0 is not background
        pos = conf_targets<19  # [N,8732], pos means the box matched.
        ...
        pos_mask = pos.unsqueeze(2).expand_as(conf_preds)  # [N,8732,20]
        neg_mask = neg.unsqueeze(2).expand_as(conf_preds)  # [N,8732,20]
        mask = (pos_mask+neg_mask).gt(0)

        pos_and_neg = (pos+neg).gt(0)
        preds = conf_preds[mask].view(-1,self.num_classes)  # [#pos+#neg,20]
