...
class _FCN8Base(nn.Module):
    def __init__(self):
        ...
        self.ssd_conv5_1 = None
        self.ssd_conv5_2 = nn.Conv2d(512, 512, kernel_size=3, padding=1, dilation=1)
        self.ssd_conv5_3 = nn.Conv2d(512, 512, kernel_size=3, padding=1, dilation=1)
...
class FCN8VGG(_FCN8Base):
    def __init__(self, num_classes, pretrained=True, phase='train'):
        ...
        self.features5_4 = nn.Sequential(*features[40:52]) # for FCN 512, 32, 32
    ...
    def forward(self, x, gt=None, train_flag=True):
        ...
        f_y4 = self.features4(f_y4_4) # 512,32,32
        f_y5_4 = self.features5_4(f_y4) # for FCN 512, 32, 32
        # pdb.set_trace()
        # SSD 
        hs = []
        hs.append(self.norm4(f_y4_4)) 

        h = F.relu(self.ssd_conv5_1(f_y4))
        h = F.relu(self.ssd_conv5_2(h))
        h = F.relu(self.ssd_conv5_3(h))
        ...
        # FCN conv5_4 + SSD conv7 
        y5 = torch.cat((f_y5_4,h), 1) # 1536, 32, 32
        y5_f = y5

        y5 = self.fconv5(y5)
        # FCN conv3
        y4 = self.fconv4(f_y4_4)
        y3 = self.fconv3(f_y3_4)
        ...
        if gt is not None: 
            # pdb.set_trace()
            pooled_features = self.roi_pool(f_y4_4, gt)
