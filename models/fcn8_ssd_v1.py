...
class FCN8VGG(_FCN8Base):
    def __init__(self, num_classes, pretrained=True, phase='train'):
        ...
        self.features3_4 = nn.Sequential(*features[0:26]) # for FCN 256,128,128
        ...
        self.features5_4 = nn.Sequential(*features[39:52]) # for FCN 512, 32, 32
        ...
        self.fconv5 = nn.Sequential(
            nn.Conv2d(1536, 2048, kernel_size=7),
    ...
    def forward(self, x, gt=None, train_flag=True):
        ...
        # features
        f_y3_4 = self.features3_4(x) # for FCN 256,128,128
        f_y4_4 = self.features4_4(f_y3_4) # 512,64,64
        f_y5_4 = self.features5_4(f_y4_4) # for FCN 512, 32, 32
        # pdb.set_trace()
        # FCN conv3
        y3_4 = self.fconv3(f_y3_4)
        y4_4 = self.fconv4(f_y4_4) # 3, 64, 64

        # SSD 
        hs = []
        hs.append(self.norm4(f_y4_4)) 

        h = F.relu(self.ssd_conv6(f_y5_4))
        h = F.relu(self.ssd_conv7(h)) # 32, 32
        hs.append(h) 

        # FCN conv5 + SSD conv7 
        y5_4 = torch.cat((f_y5_4,h), 1) # 1536, 32, 32

        # ROI Pooling
        pooled_features = []
        if gt is not None: 
            # pdb.set_trace()
            pooled_features_5_4 = self.roi_pool5_4(y5_4, gt)
            pooled_features_4_4 = self.roi_pool4_4(f_y4_4, gt)
            pooled_features_3_4 = self.roi_pool3_4(f_y3_4, gt)
            pooled_features = torch.cat((pooled_features_5_4,pooled_features_4_4,pooled_features_3_4), 1)


        y = y4_4 + F.upsample(self.fconv5(y5_4), y4_4.size()[2:], None, 'bilinear')
        y = y3_4 + F.upsample(y, y3_4.size()[2:], None, 'bilinear')
        ...
        return y, y5_4, loc_preds, conf_preds, pooled_features
...
class FCN8ResNet(_FCN8Base):
    ...
    def forward(self, x, gt=None, train_flag=True):
        ...
        if gt is not None:
            # pdb.set_trace()
            pooled_features = self.roi_pool(y4, gt)
        y4 = self.fconv4(y4)
        ...
        return y, y5_f, loc_preds, conf_preds, pooled_features
