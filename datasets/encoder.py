...
def compute_SSD_para():
    min_ratio = 20
    max_ratio = 90

    x_steps = int(math.floor((max_ratio - min_ratio) / (len(feature_map_sizes) - 2)))

    min_sizes = []
    max_sizes = []
    print 'step'
    # print xrange(min_ratio, max_ratio + 1, x_steps)


    for ratio in xrange(min_ratio, max_ratio + 1, x_steps):
        print ratio
        min_sizes.append(scale * ratio / 100.)
        max_sizes.append(scale * (ratio + x_steps) / 100.)
    min_sizes = [scale * 7 / 100.] + min_sizes
    max_sizes = [[]] + max_sizes
    print 'size'
    print min_sizes
    print max_sizes
...
class DataEncoder:
    def __init__(self):
        ...
        feature_map_sizes = (64, 32, 16, 8, 4, 2)

        scale = 512

        steps = [s / scale for s in (8, 16, 32, 64, 128, 256)]

        # sizes = [s / scale for s in (30, 60, 111, 162, 213, 264, 315)] # For SSD 300
        sizes = [s / scale for s in (50, 100, 189, 276, 364, 451, 534)] # For SSD 500
    ...
    def encode(self, boxes, classes, threshold=0.5):
        ...
        cxcy = cxcy/(variances[0] * default_boxes[:,2:])
        ...
        # print iou
        # pdb.set_trace()
        conf[iou<threshold] = 19      # background
        return loc, conf
    ...
    def decode(self, loc, conf):
        ...
        Args:
          ...
          conf: (tensor) predicted conf, sized [8732,20].
